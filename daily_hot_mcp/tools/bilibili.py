...
from daily_hot_mcp.utils import cache, http_client, logger
...
def encode_wbi(params: dict, img_key: str, sub_key: str) -> str:
    ...
    mixin_key = "".join(
        [full_key[i] if i < len(full_key) else "" for i in mixin_key_order]
    )[:32]
...
bilibili_hot_tools = [bilibili_rank_tool, bilibili_trending_tool]
