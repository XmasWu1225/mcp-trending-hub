...
async def get_sogou_trending_func() -> list:
    ...
    try:
        ...
        response = await http_client.get(
            "https://www.sogou.com/web", headers=headers, params={"query": "搜狗热搜"}
        )
        ...
        # 搜索热搜相关的元素
        hot_elements = soup.find_all(
            ["div", "span", "a"],
            class_=lambda x: x
            and any(
                keyword in x.lower()
                for keyword in ["hot", "trend", "popular", "search"]
            ),
        )
...
async def get_sogou_trending_api():
    ...
    try:
        ...
        response = await http_client.get(
            ...
            headers=headers,
            params={"type": "web", "key": "", "format": "json"},
        )
...
sogou_hot_tools = [sogou_tool_config]
