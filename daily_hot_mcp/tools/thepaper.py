...
async def get_thepaper_trending_func() -> list:
    ...
    data = response.json()
    if data.get("resultCode") != 1 or not isinstance(
        data.get("data", {}).get("hotNews"), list
    ):
    ...
    for item in data["data"]["hotNews"]:
        ...
        if tag_list:
            hashtags = " ".join(
                [f"#{tag.get('tag', '')}" for tag in tag_list if tag.get("tag")]
            )
            if hashtags:
        ...
        if item.get("contId"):
            result_item["link"] = (
                f"https://www.thepaper.cn/newsDetail_forward_{item['contId']}"
            )
...
thepaper_hot_tools = [thepaper_tool_config]
