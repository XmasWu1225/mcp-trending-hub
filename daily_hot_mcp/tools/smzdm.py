...
def _safe_json_parse(text: str) -> list | None:
    """安全解析JSON"""
    try:
...
async def get_smzdm_rank_func(
    unit: Annotated[
        int, Field(description="时间范围：1(今日热门), 7(周热门), 30(月热门)")
    ] = 1,
) -> TrendingResult:
    """获取什么值得买热门数据"""
    ...
    if unit not in valid_units:
        raise ValueError(f"不支持的时间单位: {unit}，有效值为: {valid_units}")
    ...
    try:
        ...
        if "application/json" not in content_type and "text/json" not in content_type:
            logger.warning(f"什么值得买返回非JSON响应，内容类型: {content_type}")
            return []

        response.encoding = "utf-8"
        text = response.text.strip()
        if not text:
            logger.warning("什么值得买返回空响应")
            return []

        data = response.json()
        if data.get("error_code") != 0 or not isinstance(data.get("data"), list):
            error_msg = data.get("error_msg", "获取什么值得买热门失败")
            logger.error(f"什么值得买API错误: {error_msg}")
            return []
        ...
        for item in data["data"]:
            result_item = {
                ...
                "popularity": str(item.get("collection_count", 0)),
                "link": item.get("article_url", ""),
            }

            tag_data = _safe_json_parse(item.get("tag", ""))
            if tag_data and isinstance(tag_data, list):
                hashtags = " ".join(
                    [f"#{tag.get('title', '')}" for tag in tag_data if tag.get("title")]
                )
                if hashtags:
        ...
        logger.error(f"获取什么值得买数据失败: {str(e)}")
        return []
...
smzdm_hot_tools = [smzdm_tool_config]
